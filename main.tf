...
resource "azurerm_mssql_server" "primary_server_one" {
  name                         = "devdb-sqlserver-one"
  resource_group_name          = azurerm_resource_group.rg.name
  location                     = azurerm_resource_group.rg.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "Dev_DB" {
  name         = "DevDB"
  server_id    = azurerm_mssql_server.primary_server_one.id
...
resource "azurerm_mssql_server" "sec" {
  name                         = "sec-sql"
...
resource "azurerm_mssql_failover_group" "fg" {
  ...
  partner_server {
    id = azurerm_mssql_server.sec.id
  }
